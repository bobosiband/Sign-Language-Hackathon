...
iklayenti = TestClient(app)


def test_impilo_ibuyisela_impendulo_ephumelele():
    impendulo = iklayenti.get("/health")
    assert impendulo.status_code == 200
    assert impendulo.json()["isimo"] == "kuhamba kahle"


def test_imininingwane_yemodeli_ibuyisela_imininingwane_efanele():
...
def test_izinhlamvu_ezitholakalayo_zibuyisela_uhlu_oluzeleyo():
...
def test_i_websocket_ibuyisela_ukubikezela_okuqondile():
...
def test_i_websocket_iphatha_ubude_obungalungile_ngaphandle_kokuphahlazeka():
    with iklayenti.websocket_connect("/ws") as isokhethi:
...
def test_i_websocket_iphatha_idatha_engelona_uhlu_ngaphandle_kokuphahlazeka():
    with iklayenti.websocket_connect("/ws") as isokhethi:
...
def test_ukuthumela_isibonelo_nokuphinda_ukuqeqesha():
...
def test_ukuthumela_isibonelo_esinobude_obungalungile_kubuyisela_iphutha():
    impendulo = iklayenti.post(
        "/api/samples",
